...
exports.vendorForgotPassword = async(req,res,next) => {
    try {
        var userExists = await vendorRegister.findOne({email: req.body.email.toLowerCase()});
        console.log(userExists);
        if(userExists != "" && userExists != null)
        {
            var str = generateP()
            var cipher = crypto.createCipher(algorithm, key);  
            var encryptedData = cipher.update(str, 'utf8', 'hex') + cipher.final('hex');
            await vendorRegister.updateOne({ email: req.body.email.toLowerCase() }, { $set: { password: encryptedData } })
            await mailFunction.send_mail(req.body.email, "Forgot Password", "Your Updated password is "+str);
            res.status(200).json({
                status: message.messages.TRUE,
                message: "Forgot password successfully. Please Check your mail for New Password"
            })
        }
        else
        {
