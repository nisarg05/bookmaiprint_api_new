...
async function order_details(order_id) {
    ...
    if(order_info)
    {
        ...
        for(let i = 0; i < order_details_info.length; i++)
        {
            ...
            let allocate_order_info = await allocateOrderModel.findOne({order_id: order_id, order_details_id: order_details_info[i]._id, deleted_at: null});
            console.log("allocate_order_info",allocate_order_info)
