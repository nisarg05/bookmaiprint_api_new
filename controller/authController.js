...
const Vendoruser = require('../model/vendorRegister');
...
exports.changeVendorPassword = async (req, res, next) => {
    ...
    try {
        ...
        if (user_id) {
            var userExists = await Vendoruser.findOne({ _id: user_id });
            if (userExists != "" && userExists != null) {
                ...
                if (decrypted == req.body.password) {
                    ...
                    var encryptedData = cipher.update(req.body.new_password, 'utf8', 'hex') + cipher.final('hex');
                    await Vendoruser.updateOne({ email: userExists.email.toLowerCase() }, { $set: { password: encryptedData } })
...
async function user_details(id) {
    ...
    if (userExists && userExists._id != "undefined" && userExists._id != undefined && userExists._id != "null" && userExists._id != "") {
        var addressExists = await address.findOne({ user_id: userExists._id, deleted_at: null }).sort({ _id: -1 });
