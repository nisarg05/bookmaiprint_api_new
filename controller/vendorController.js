...
exports.vendorByCategory = async(req,res,next) => {
    try {
        ...
        var questionsExists = await priceListModel.find({_id: req.body.category_id, deleted_at: null}).limit(limit * 1).skip((page - 1) * limit).sort({_id: -1});

        let result = [];
        for(let i = 0; i < questionsExists.length; i++)
        {
            if (questionsExists[i].vendor.length > 0) {
                for(let s = 0; s < questionsExists[i].vendor.length; s++)
                {   
                    if(questionsExists[i].vendor[s].is_active === true)
                    {
                        let vendorDetails = await vendorRegisterController.user_details_price(questionsExists[i].vendor[s]);
...
exports.assignedOrderListByVendor = async(req,res,next) => {
    try {
        ...
        let allocateOrderList = await allocateOrderModel.find({vendor_id:id}).sort({_id:-1});
        let result = [];
...
async function count_vendor_id(category_id) {
    const count = await priceListModel.find({_id: category_id, deleted_at: null}).count();
    return count;
}
