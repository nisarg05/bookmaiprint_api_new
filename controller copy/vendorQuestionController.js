...
exports.getQuestionsAdmin = async(req,res,next) => {
    try {
        ...
        var questionList = await questionModel.find({vendor_id: user_id, deleted_at: null}).limit(limit * 1).skip((page - 1) * limit).sort({_id: -1}), result = [];
...
async function count_qustions(vendor_id) {
    const count = await questionModel.find({vendor_id: vendor_id, deleted_at: null}).count();
    return count;
}
...
async function get_question_info(data) {
    ...
    if(data)
    {
        ...
        let vendorInfo = await vendorModel.findOne({_id: data.vendor_id, deleted_at: null});
        ...
        result = {
            ...
            vendor_id: data.vendor_id != undefined && data.vendor_id != null && data.vendor_id != "" && vendorInfo != null ? vendorInfo._id : null,
            vendor_info: data.vendor_id != undefined && data.vendor_id != null && data.vendor_id != "" && vendorInfo != null ? vendorInfo : null,
        }
    }
